...
### 로컬 API key 암호화 secret

기존 로컬 개발 DB를 계속 사용할 때는 `API_KEY_ENCRYPTION_SECRET`을 고정해야 합니다. 기본 로컬 값은 다음과 같습니다.

```bash
API_KEY_ENCRYPTION_SECRET=development-only-api-key-secret
```

Provider key는 DB에 암호화되어 저장됩니다. 같은 로컬 DB 파일을 유지한 채 서버를 다른 `API_KEY_ENCRYPTION_SECRET`으로 다시 띄우면 기존 provider key를 복호화할 수 없어 provider 호출 전에 스토리보드/목업 생성이 실패합니다. secret을 의도적으로 바꾼 경우에는 관리자 화면에서 provider key를 삭제 후 다시 저장하거나, 새 로컬 DB를 만들어야 합니다.
